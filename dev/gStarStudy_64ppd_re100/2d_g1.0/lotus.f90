...
! -- physical parameters
  ...
  real,parameter    :: g_star = 1.0 ! nondimensional spacing
...
! -- numerical parameters
  ...
  real,parameter  :: finish = 1500*D ! finish time
...
! -- grid parameters
  ...
  integer,parameter :: noDims = 2        ! no. of dims of the simulation
!
! -- twist parameters
...
!
  ...
  call flow%write()
	...
	call gatherWrite2D(flow%velocity,n,b)
...
! -- make an array of cylinders with group diameter DG
  ...
  subroutine gatherWrite2D(u,n,b)
  	use fieldMod, only: field
  	use vectorMod, only: vfield
  	use mympiMod
  	type(vfield),intent(in) :: u
  	integer,intent(in)			:: n(3),b(3)
  	integer :: is,ie,js,je,ks,ke,mx,my
  	integer :: i,j,k,m(3),p,nproc,my_rank,index
  	logical :: root
  	real :: pos(3)
  	...
  	if(root) print *, '-- In Gather Write subroutine ... --'
  	...
  	if(root) print *, '-- Writing procs data into files ... --'
  	write(m_char,'(I4)') my_rank
  	file = 'output'//trim(adjustl(m_char))//'.dat'
  	open(my_rank, file = file, status = 'replace', action = 'write', form = 'unformatted', access='stream')
  	do j = js,je
  		do i = is,ie
  			write(my_rank) u%e(1)%p(i,j,1), u%e(2)%p(i,j,1)
  		end do
  	end do
  	close(my_rank)
  	...
  	if(root) then
  		print *, '-- Initializing v(n) field ... --'
  		call v%init(n)
  		print *, '-- Reading procs data ... --'
  		do p=0,nproc-1
  			mx = int(p/(b(3)*b(2)))
  			my = mod(int(p/b(3)),b(2))
  			! print*, p, mx,my
  			write(m_char,'(I4)') p
  			file = 'output'//trim(adjustl(m_char))//'.dat'
  			!print *, file

  			open(97, file = file, status = 'old', action = 'read', form = 'unformatted', access='stream')
  			do j = js,je
  				do i = is,ie
  					read(97) v%e(1)%p(i+m(1)*mx,j+m(2)*my,1), &
  											v%e(2)%p(i+m(1)*mx,j+m(2)*my,1)
  				end do
  			end do
  			close(97)
  		...
  		write(*,*) 'Writing in /output/ ...'
  		call v%e(1)%limits(is,ie,js,je,ks,ke)
  		dir = 'output/'
  		index = scan(m_char,'.')
  		s1 = m_char(1:index-1)
  		s2 = m_char(index+1:)
  		...
  		file = 'gather_output_2D.dat'
  		open(99, file=file, status = 'replace', action = 'write', form = 'unformatted', access='stream')
  		do j = js,je
  			do i = is,ie
  				write(99) v%e(1)%p(i,j,1), v%e(2)%p(i,j,1)
  			end do
  		end do
  		close(99)

  		file = 'output_tecplot.dat'
  		open(99, file=file, status="replace", action="write")
  		write(99,*)'TITLE = "flow field"'
  		write(99,*)'VARIABLES = "i" "j" "k" "x" "y" "z" "u"'
  		write(99,*)'ZONE T = "flow field", I=',ie-is+1,' J=',je-js+1,' K=',1,', DATAPACKING=POINT'
  		do k = ks,ks
  			do j = js,je
  				do i = is,ie
  					pos(1) = xg(1)%x(i-2)
  					pos(2) = xg(2)%x(j-2)
  					pos(3) = 1.
  					write(99,102) i, j, k, pos(1), pos(2), pos(3), v%e(1)%p(i,j,k)
  				end do
  			end do
  		end do
  		close(99)
  		102 format(3I6,3F12.2,1E14.6)
  	end if

  	! call mympi_wait()

  end subroutine gatherWrite2D
  ...
  subroutine gatherRead2D(u,n,b)
  	...
  	if(root) print *, '-- In Gather Read subroutine ... --'
    ...
    print*, m
  	...
  	open(my_rank, file = file, status = 'old', action = 'read', form = 'unformatted', access='stream')
  			...
  			skip_blocs = my*m(2)*n(1) ! Skipe entire row of blocs
        print*, 'skip_blocs',my_rank, skip_blocs, my, m(2), n(1)
  			do k = 1, skip_blocs
  				read(my_rank) dum1, dum2
  			end do

        skipx1 = mx*m(1)
        print*, 'skipx1',my_rank, skipx1, mx, m(1)
        skipx2 = (b(1)-mx-1)*m(1)
        print*, 'skipx2',my_rank, skipx2, b(1), mx, m(1)
  			...
  			do j = js,je
  				...
  				do i = is,ie
  					read(my_rank) u%e(1)%p(i, j, ks), u%e(2)%p(i, j, ks)
  				end do
